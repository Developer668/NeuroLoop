...
The persistent Windows execution hold remains after repeated 0x113 graphics crashes. The Apple Silicon launcher has a separate explicit guarded MPS path that preserves the hold and enforces memory headroom. This table distinguishes completed implementation from unpassed acceptance.

| Work | Current state | Evidence still required |
|---|---|---|
| Isolated pinned environments | App/model staged runtimes pass dependency checks; 119 runnable backend tests, frontend build/typecheck and asset verification pass | One fresh uncached full-loop run using the current memory-first Mac V-JEPA2 path; final multimodal stability remains unpassed |
...
| Capability/documentation drift | Windows hold and Mac override are now documented separately; weight presence remains separate from inference proof | Keep statuses tied to new evidence when runtime/model versions change |
...
Next Windows step: obtain dump analysis using [CRASH-RECOVERY.md](CRASH-RECOVERY.md). Do not remove the hold merely to finish an integration test. Next Mac step: close other heavy applications, launch through `Start-NeuroLoop.sh`, confirm memory headroom, and run one bounded fresh uncached full-loop acceptance test. Launch remains a separate gate. Scientific and commercial activation require their separate evidence.
