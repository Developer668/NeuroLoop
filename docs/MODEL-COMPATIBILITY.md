...
The earlier multimodal length mismatch was resolved by preserving the shared 2 Hz grid. Audio and compact video feature pooling use CPU after the configured layer-group mean is computed, and each neural evaluation runs in its own supervised child process. The new preflight refuses an already hot GPU (82°C or higher), less than 4 GiB free GPU memory, or less than 3 GiB free system RAM. PyTorch's 75% allocation limit, bounded execution, cancellation and non-replay recovery remain in place.
