...
def profile_id() -> str:
    root=settings().root
    files=[root/'models/load_local_tribe.py',root/'tribev2-balanced-qv-local/load_quantized_tribev2.py',root/'tribev2-balanced-qv-local/config.yaml',root/'tribev2-balanced-qv-local/quantized_video/quantization.json',Path(__file__)]
    h=hashlib.sha256()
    for p in files:
