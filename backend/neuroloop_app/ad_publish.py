...
class AdPublishService:
    ...
    def sync_asset(self, request: AdAssetSyncRequest) -> dict:
        ...
        try:
            ...
            if hashlib.sha256(data).hexdigest() != asset_record["sha256"]:
                raise AdProviderFailure("ASSET_INTEGRITY_FAILED", "Stored media does not match its recorded hash. Upload was not sent.")
            token = self._access_token(connection)
    ...
    def _upload_meta(self, token: str, account_id: str, asset: dict, data: bytes, label: str) -> dict:
        ...
        try:
            ...
            return {"kind": "video", "video_id": video_id, "processing": "not_checked"}
    ...
    def check_meta_video(self, publication_id: str) -> dict:
        with self.store.read() as session:
            publication = required(session, AdPublication, publication_id)
            if publication.provider != "meta" or not publication.remote.get("video_id"):
                raise DomainError("This receipt has no Meta video to check", 422)
            connection_id = publication.connection_id
            video_id = publication.remote["video_id"]
        connection = self._connection("meta")
        if not connection or connection.id != connection_id:
            raise DomainError("Reconnect the Meta authorization used for this upload", 409)
        result = self._meta_get(self._access_token(connection), "/" + video_id, {"fields": "status"})
        status = result.get("status", {}).get("video_status")
        if not isinstance(status, str) or not status:
            raise AdProviderFailure("MISSING_STATUS", "Meta did not return video processing status.")
        with self.store.transaction() as session:
            publication = required(session, AdPublication, publication_id, lock=True)
            publication.remote = {**publication.remote, "processing": status, "processing_checked_at": now()}
