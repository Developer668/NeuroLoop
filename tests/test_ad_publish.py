...
@pytest.mark.parametrize("processing", ["processing", "ready", "error"])
def test_meta_processing_readback_preserves_upload_receipt(state, settings, tmp_path, processing):
    store, objects = state
    _, asset = _image_asset(store, objects, tmp_path)
    def handler(request):
    ...
    with store.transaction() as session:
        connection = AdConnection(provider="meta", active=True, access_token_ciphertext=TokenVault(settings).seal("token"), scopes=[], details={})
        session.add(connection)
        session.flush()
        receipt = AdPublication(provider="meta", connection_id=connection.id, campaign_id=asset["campaign_id"], asset_id=asset["id"], account_id="act_123", idempotency_key="processing-test", spec={}, state="SYNCED", remote={"video_id": "12345"})
        session.add(receipt)
        session.flush()
        identity = receipt.id
    result = service.check_meta_video(identity)
    assert result["remote"]["processing"] == processing
    assert result["remote"]["video_id"] == "12345"
    assert result["remote"]["processing_checked_at"] > 0
    assert result["state"] == "SYNCED"  # The upload receipt survives an encoding failure.


def test_meta_upload_sends_exact_video_bytes(state):
    store, objects = state
    data = b"explicit HTTP contract fixture, not a generated video"
    def handler(request):
        ...
        assert b'filename="creative.mp4"' in request.content
        return httpx.Response(200, json={"id": "12345"})
    service = AdPublishService(store, objects, httpx.Client(transport=httpx.MockTransport(handler)))
    receipt = service._upload_meta("token", "act_123", {"kind": "video", "name": "creative.mp4", "mime": "video/mp4"}, data, "Creative")
    assert receipt == {"kind": "video", "video_id": "12345", "processing": "not_checked"}
